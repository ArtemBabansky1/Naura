 ...
 * missing WebGL. */
function PhoneCanvas({ chat, progress }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      unsubscribeProgress = progress.on("change", applyProgress)
...
export default function MeetsPhone() {
  ...
  // Rise and rotation share the panel's progress and finish before its centre
  // reaches the middle of the screen, leaving time to read the settled chat.
  const { scrollYProgress } = useScroll({
    target: sectionRef,
    offset: ["start 95%", "center 60%"],
  })
  const flyY = useTransform(scrollYProgress, [0, 1], ["45%", "0%"])
  ...
  return (
    ...
    >
      <div className="container meets-phone__panel meets-dark" ref={sectionRef}>
        <ConnectionLines />
        <div className="meets-phone__grid">
        <motion.div
          className="meets-phone__fly"
          style={prefersReduced ? undefined : { y: flyY }}
        >
          <PhoneCanvas key={i18n.language} chat={chat} progress={scrollYProgress} />
        </motion.div>

        <motion.div className="meets-phone__text" {...textMotion}>
          <h2 className="meets-phone__title text-h2">
            <RevealText text={t("phone.headline")} accent={t("phone.headline").split(/ +/).at(-1)} />
          </h2>
