...
function CardPhoto({ photo }) {
  ...
  return (
    <picture className="meets-how__media">
...
export default function MeetsHow() {
  ...
  useLayoutEffect(() => {
    ...
    // Size the cards so exactly four align with the header container, and
    // inset only the leading edge. The track contains no trailing empty cell.
    const layout = () => {
    ...
    // Measure the sixth card directly: the pin ends the moment its right edge
    // reaches the header container, independent of track padding or scrollbars.
    const distance = () => {
      const lastCard = track.lastElementChild
      if (!lastCard) return 0
      const trackRect = track.getBoundingClientRect()
      const lastRect = lastCard.getBoundingClientRect()
      return Math.max(0, lastRect.right - trackRect.left - head.getBoundingClientRect().right)
    }
