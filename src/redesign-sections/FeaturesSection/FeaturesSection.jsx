...
import { FeatureVisual } from '../../pages/BusinessCardsPage/FeatureIllustrations'
import { CrmFeatureVisual } from './CrmFeatureVisuals'
...
function HandshakeChain() {
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="516" height="168" fill="none" viewBox="0 0 516 168" className="hsc">
      ...
      <path className="hsc-fill--solid" clipPath="url(#hsc-wipe-solid)" stroke="var(--text-primary)" strokeLinecap="round" strokeWidth="1.3" d="M100.5 47.9h93" />
      <path className="hsc-fill--dashed" clipPath="url(#hsc-wipe-dashed)" stroke="var(--text-primary)" strokeDasharray="8.09 8.09" strokeLinecap="round" strokeWidth="1.3" d="M308.1 47.9h93.1" />

      {/* Stage 1 — You */}
      <g className="hsc-stage hsc-stage--1">
        <circle className="hsc-dot" cx="43.1" cy="43.1" r="43.1" fill="var(--bc-olive)" />
        <text className="hsc-initial" x="43.1" y="43.1" textAnchor="middle" dominantBaseline="central" fontSize="32" fontWeight="500" fill="var(--text-primary)">{t(`${base}.you.initial`)}</text>
        <text className="hsc-name" x="43.1" y="118" textAnchor="middle" fontSize="13" fontWeight="500" fill="var(--text-primary)">{t(`${base}.you.name`)}</text>
      </g>

      {/* Stage 2 — Anna */}
      <g className="hsc-stage hsc-stage--2">
        <circle className="hsc-dot" cx="250.8" cy="43.1" r="43.1" fill="var(--bc-olive)" />
        <text className="hsc-initial" x="250.8" y="43.1" textAnchor="middle" dominantBaseline="central" fontSize="32" fontWeight="500" fill="var(--text-primary)">{t(`${base}.anna.initial`)}</text>
        <text className="hsc-name" x="250.8" y="118" textAnchor="middle" fontSize="13" fontWeight="500" fill="var(--text-primary)">{t(`${base}.anna.name`)}</text>
      ...
      <g className="hsc-stage hsc-stage--3">
        <circle className="hsc-dot" cx="458.5" cy="43.1" r="43.1" fill="var(--bc-olive)" />
        <text className="hsc-initial" x="458.5" y="43.1" textAnchor="middle" dominantBaseline="central" fontSize="32" fontWeight="500" fill="var(--text-primary)">{t(`${base}.target.initial`)}</text>
        <text className="hsc-name" x="458.5" y="118" textAnchor="middle" fontSize="13" fontWeight="500" fill="var(--text-primary)">{t(`${base}.target.name`)}</text>
        <text className="hsc-sub" x="458.5" y="143" textAnchor="middle" fontSize="12" fill="var(--bc-olive)">{t(`${base}.target.sub`)}</text>
      </g>
    </svg>
  )
}
    ...
    <svg width="377" height="246" viewBox="0 0 377 246" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <defs>
        <linearGradient id="rc-paint0-linear" x1="188.5" y1="15" x2="188.5" y2="244" gradientUnits="userSpaceOnUse">
          <stop stopColor="var(--bc-olive)" />
          <stop offset="1" stopColor="var(--bc-paper)" />
        </linearGradient>
      </defs>
    </svg>
  )
...
/* CRM-specific UI scenes keep the approved digital-card visual language while
 * mapping every illustration directly to its CRM capability. */
const ROW_A = [
  { key: 'contactGraph', size: 'md', media: 'visual', referenceVisual: 3 },
  { key: 'handshake',    size: 'md', media: 'visual', scene: 'handshakePath' },
  { key: 'telegramBot',  size: 'sm', media: 'visual', scene: 'telegramSave' },
]

const ROW_B = [
  { key: 'mcpAgents',  size: 'sm', media: 'visual', scene: 'mcpNetwork' },
  { key: 'workspaces', size: 'md', media: 'visual', referenceVisual: 1 },
  { key: 'reminders',  size: 'md', media: 'visual', referenceVisual: 4 },
]
...
function FeatureCard({ card, index }) {
  ...
  return (
    <motion.article
      ...
      transition={reduced
        ? { duration: 0 }
        : { visualDuration: 0.6, type: 'spring', bounce: 0.16, delay: index * 0.04, restDelta: 0.001 }}
      className={[
        'feature-card',
    ...
    >
      ...
      {(card.scene || card.referenceVisual !== undefined) && (
        <div className="feature-card__media" aria-hidden="true">
          {card.referenceVisual !== undefined
            ? <FeatureVisual index={card.referenceVisual} />
            : <CrmFeatureVisual scene={card.scene} />}
        </div>
      )}
    </motion.article>
  )
    ...
    <section id="features" data-section="features" className="features-section section">
      <div className="container">
        <header className="features-header">
          <RevealTitle as="h2" className="features-header__title text-h2" text={t('headline')} />
          <RevealCopy as="p" className="features-header__subtitle text-body-lg" delay={0.12}>{t('subtitle')}</RevealCopy>
        </header>

        <div className="features-grid">
          <div className="features-row features-row--a">
            {ROW_A.map((card, index) => (
              <FeatureCard key={card.key} card={card} index={index} />
            ))}
          </div>
          <div className="features-row features-row--b">
            {ROW_B.map((card, index) => (
              <FeatureCard key={card.key} card={card} index={index + ROW_A.length} />
            ))}
          </div>
        </div>
      </div>
