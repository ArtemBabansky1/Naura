...
const C = {
  ...
  list: '#484843',
  meta: '#777671',
  accent: PRODUCT_COLORS.crm.accent,
  accentInk: PRODUCT_COLORS.crm.ink,
  chip: '#ebeae6',
  borderSubtle: 'rgba(11, 11, 11, 0.18)',
  borderFaint: 'rgba(11, 11, 11, 0.12)',
}
...
function drawDirectoryScreen(ctx, W, H, photos) {
  ...
  pill(ctx, chipX, chipY, chipW, chipH, { fill: C.chip, stroke: C.borderFaint, lineWidth: 2 * s })
  ...
  fade.addColorStop(0, 'rgba(244,243,241,0)')
  ...
  MEMBERS.forEach((m, i) => {
    ...
    const askY = y + ip + 6 * s
    pill(ctx, askX, askY, askW, askH, { fill: C.chip, stroke: C.borderFaint, lineWidth: 2 * s })
    ctx.fillStyle = C.list
    ctx.textAlign = 'left'
    ctx.textBaseline = 'middle'
    ...
    pill(ctx, tagX, tagY, tagW, tagH, { fill: C.chip, stroke: C.borderFaint, lineWidth: 2 * s })
    ctx.fillStyle = C.meta
    ctx.textAlign = 'center'
    ctx.textBaseline = 'middle'
  ...
  TABS.forEach((label, i) => {
    const cx = tabW * (i + 0.5)
    const active = i === 0
    const color = active ? C.accentInk : C.muted
