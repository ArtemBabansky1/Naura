...
import { HiwScrollCard, HiwScrollCopy, HiwScrollNumeral, useHiwScrollProgress } from './HiwScrollMotion'
import { RevealTitle } from '../../components/ScrollReveal/ScrollReveal'
...
// Each numeral starts beige. Its upper stop transitions to olive only after
// that step is activated; the lower stop stays beige. One gradient id per step
...
const NUMERAL_GRAD = (id) => (
  <linearGradient id={id} x1="0" y1="1.5" x2="0" y2="63.5" gradientUnits="userSpaceOnUse">
    <stop className="hiw-num__gradient-top" stopColor="var(--bc-canvas)" />
    <stop offset="1" stopColor="var(--bc-canvas)" />
  </linearGradient>
)
  ...
  return (
    <div className="container">
      <header className="hiw-header">
        <RevealTitle as="h2" className="hiw-header__title text-h2" text={t('headline')} />
      </header>
    </div>
  )
}
...
function StepText({ index, scrollProgress }) {
  const { t } = useTranslation('howItWorks')
  const key = STEP_KEYS[index]
  const numeral = <StepNumeral index={index} label={t(`steps.${key}.number`)} />
  const copy = (
    <>
  ...
  return (
    <>
      {scrollProgress ? (
        <HiwScrollNumeral index={index} progress={scrollProgress}>
          {numeral}
        </HiwScrollNumeral>
      ) : numeral}
      {scrollProgress ? (
        <HiwScrollCopy index={index} progress={scrollProgress}>
          {copy}
        </HiwScrollCopy>
      ) : copy}
      {index === 0 && (scrollProgress ? (
        <HiwScrollCard progress={scrollProgress}>
          <ContactCard />
        </HiwScrollCard>
      ) : <ContactCard />)}
    </>
...
const readStaticMode = () => {
  ...
  return window.matchMedia(REDUCED_QUERY).matches || !window.matchMedia(DESKTOP_QUERY).matches
}
...
export default function HowItWorksSection() {
  ...
  useEffect(() => {
    ...
    const sync = () => setStaticMode(reducedMq.matches || !desktopMq.matches)
      ...
      <div className="hiw-scroller" ref={scrollerRef}>
        <div className="hiw-pin" ref={pinRef}>
          ...
          <div className="hiw-text-col">
            {STEP_KEYS.map((key, i) => (
              <div className="hiw-text" key={key}>
                <StepText index={i} scrollProgress={scrollProgress} />
              </div>
            ))}
          </div>
