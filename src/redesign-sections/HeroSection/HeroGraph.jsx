...
export default function HeroGraph() {
  ...
  useLayoutEffect(() => {
    ...
    if (reduced) {
      NODES.forEach(n => {
        ...
        if (n.id === 'mary') props.boxShadow = 'none'
        gsap.set(el, props)
      })
      EDGES.forEach(([a, b]) => {
    ...
    NODES.forEach(n => {
      const el = nodeEls.current[n.id]
      if (el) gsap.set(el, { opacity: 0, scale: (DEPTH_SCALE[n.depth] ?? 1) * 0.82, transformOrigin: 'center center' })
    })
    EDGES.forEach(([a, b]) => {
    ...
    // Start with the headline: the web opens as one fast, continuous wave and
    // eases gently into its final state instead of waiting for a scroll trigger.
    ...
    // Each ring starts before the previous one finishes. Absolute offsets keep
    // the complete reveal near 1.5s even when a ring contains more edges.
    EDGE_GROUPS.forEach((group, groupIdx) => {
      const tier = groupIdx + 1
      const tierStart = 0.05 + groupIdx * 0.23
      ...
      tierNodeIds.forEach((nodeId, i) => {
        ...
        tl.to(
          el,
          { opacity: 1, scale: DEPTH_SCALE[nodeMap[nodeId]?.depth ?? 1], duration: 0.38, ease: 'power3.out' },
          tierStart + 0.1 + i * 0.035
        )
      })
    })
  ...
  // ── CTA collision guard ───────────────────────────────────────────────────
  // Cards that share the CTA's horizontal band establish the collision-safe
  // baseline. The whole composition is then optically lifted by 70px so its
  // bottom rows remain in view; side cards may rise alongside the CTA.
  useLayoutEffect(() => {
    ...
    function applyOffset() {
      ...
      const collisionOffset = Math.max(0, Math.round(ctaBottom + gap - minCentralTop))
      const offset = collisionOffset - GRAPH_LIFT_PX
